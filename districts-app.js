...
(function () {
  ...
  U.forEach(function (u, i) {
    ...
    var nm = u.names31.length ? u.names31 : u.members.map(function (m) { return m.area; });
  ...
  // 1931 census territory underneath: whatever no district unit covers shows hatched
  Object.keys(G.units).forEach(function (k) {
    el('path', { d: G.units[k].d, class: 'extra' }, $('#g-base'));
    el('path', { d: G.units[k].d, class: 'prov-line' }, $('#g-prov'));
  });
  Object.keys(G.extra).forEach(function (k) { el('path', { d: G.extra[k].d, class: 'extra' }, $('#g-extra')); });
  el('path', { d: G.modern, class: 'modern' }, $('#g-modern'));
  var unitEls = [];
  ...
  $('#opt-modern').addEventListener('change', function () { stage.classList.toggle('show-modern', this.checked); });
  ...
  function tipHTML(u) {
    ...
    var sn = statusNote(u);
    if (u.status === 'nodata' || u.status === 'nc') return h + '<span class="m">' + esc(sn) + '</span>';
    if (u.damaged) return h + '<span class="m">' + esc(statusNote(u)) + '</span>';
    if (!itemised(u)) return h + '<span class="m">' + esc(COARSE) + '</span>';
    if (state.mode === 'lang') {
      ...
      h += '<span class="big">' + esc(disp(u.lead.l)) + '</span><span class="m">' + pct(u.lead.n / u.P * 100) + ' of ' + num(u.P) + '</span>';
    }
  ...
  function render() {
    ...
    U.forEach(function (u) {
      ...
      p.classList.remove('zero', 'sel'); p.style.fill = '';
      if (u.status === 'nodata' || u.status === 'nc' || !itemised(u)) { p.style.fill = 'url(#hatch)'; return; }
      var v = value(u);
      if (!v) p.classList.add('zero'); else p.style.fill = 'var(' + SEQ[bin(sc, v)] + ')';
      if (u.missing31.length || u.gaps.length) p.classList.add('partial');
    });
    ...
    U.slice().sort(function (a, b) { return b.P - a.P; }).forEach(function (u) {
      ...
      var txt = null;
      if (state.mode === 'lead') txt = u.lead ? disp(u.lead.l).replace(/ \(as printed\)$/, '').replace(/, Hindi or Urdu$/, '').replace(/ or .*$/, '').replace(/ Group$/, '') : null;
    ...
    h += '<div class="row"><span class="sw hatch"></span> no district figures (not enumerated, outside the census, not separable, or not itemised in Burma)</div>';
    $('#legend').innerHTML = h;
  ...
  function renderSummary() {
    ...
    h += '</div>';
    if (l.note) h += '<div class="note-box">' + esc(l.note) + '</div>';
  ...
  function renderUnit() {
    ...
    if (u.members.length) {
      h += '<div class="subunits"><span class="lbl">1931 areas in this unit</span><ul class="members">' + u.members.map(function (m) {
  ...
  function popBlock(u) {
    ...
    v.forEach(function (x, i) {
      ...
      svg += '<g><title>' + y + ': ' + (x == null ? 'not available' : num(x)) + '</title>' +
    ...
    h += '<div class="cite">2011 districts: ' + esc(u.d2011.join(', ')) + '.</div></div>';
    return h;
  }
  ...
  function renderTable() {
    ...
    var rows = U.filter(function (u) { return u.P && itemised(u); }).map(function (u) {
      ...
      return { u: u, c: [u.short, u.P, u.lead ? disp(u.lead.l) : '–', u.lead ? u.lead.n / u.P * 100 : 0] };
    });
  ...
  /* ---------------------------------------------------------------- interaction */
  function select(i) { state.unit = state.unit === i ? null : i; $('#unit-panel').removeAttribute('data-all'); render(); }
  ...
  var quick = [HB, BYID[261], BYID[226], BYID[256], BYID[211], BYID[273], BYID[270], BYID[259], BYID[213]].filter(Boolean);
  $('#quick').innerHTML = quick.map(function (l) { return '<button type="button" data-lang="' + l.id + '">' + esc(l === HB ? 'Hindi composite' : disp(l).replace(/ or .*$/, '')) + '</button>'; }).join('');
